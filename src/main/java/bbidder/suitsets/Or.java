...
public final class Or extends SuitSet {
    private final SuitSet s1;
    private final SuitSet s2;

    public Or(SuitSet s1, SuitSet s2) {
        super();
        this.s1 = s1;
        this.s2 = s2;
    }
    ...
    public MyStream<Context> resolveSuits(SuitTable suitTable) {
        return s1.resolveSuits(suitTable)
                .flatMap(e1 -> s2.resolveSuits(e1.suitTable).map(e2 -> new Or(e1.getSuitSet(), e2.getSuitSet()).new Context(e2.suitTable)));
    }
